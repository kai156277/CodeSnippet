...
void ScanCheckPage::PrintPageParam(PageParam *param)
{
    ...
    for (it = dp->datFileResultMap.begin(); it != dp->datFileResultMap.end(); it++)   //依次处理每个扫描仪
    {
        ...
        if (!dp->bSingle)
        {
            tmpStr.append(QStringLiteral("<tr><th >%1参数</th> <th >%2</th> </tr>").arg(m_bMultiBeam ? QStringLiteral("多波束") : QStringLiteral("扫描仪")).arg(strScan));
...
ScanCheckPage::ScanCheckPage(const QString &title, bool mb, QWidget *parent /*= 0*/)
    : CheckPage(title, parent)
{
    m_bMultiBeam = mb;
}
