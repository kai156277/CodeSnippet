    ...
    , ui(new Ui::ScanFilterPage)
{
    ...
    onRadioToggled(true);
    //    connect(ui->radio_BeijingTime, &QAbstractButton::toggled, this, &ScanFilterPage::onRadioToggled);    //
    //    connect(ui->radio_RelativeTime, &QAbstractButton::toggled, this, &ScanFilterPage::onRadioToggled);   //
...
void ScanFilterPage::on_pushButton_time_clicked()
{
    ...
    if (cd.dMin > maxtm || cd.dMax < mintm)
    {
        ...
        if (cd.dMin > maxtm || cd.dMax < mintm)
        {
            //            interfacePrompt::Warn(this, m_titleHint, QStringLiteral("时间范围和POS不一致\nPOS时间为%1～%2").arg(SEC2HMS(mintm, ParamInterface::getHourShift())).arg(SEC2HMS(maxtm, ParamInterface::getHourShift())));
            return;
        }
    }
